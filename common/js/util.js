...
function gameRand(json,level){
	...
	let arr = Array.from({length:count/3}, v=> Math.floor(Math.random()*(parseInt(level)+2))+1);
...
//地图16*16
export const gameMap = (name,level=1)=>{
	let json;
	if(level>1){
		json = require('@/common/data/game2.json')
	}else{
		json = require('@/common/data/game1.json')
	}
	
	let arr=[]
	let view_stack={} // 视图堆栈,满足上面有四个点位，就显示
	let rand_arr = gameRand(json,level);
	for(let i = 0; i<json.length; i++){
		for(let j = 0; j<json[i].length; j++){
